...
const getBooksData= async function (req, res) {
     ...
     let allBooks= await BookModel.find({bookName:A1})

     //let allBooks= await BookModel.find( { authorName : "a1" , isPublished: true  } ) // AND

    //let allBooks = await BookModel.find({
    //    $or: [{ authorName: "a1" }, { isPublished: true }, { "year": 2022 }]
    //}).select({ bookName: a1, authorName: aa1, _id: 0 })// SELECT keys that we want
    ...
    //  let allBooks= await BookModel.find({     sales : {$gt: 20, $lt: 100}   })  //sales is between 20 and 100.... sales > 20 AND sales <100
     ...
     //let allBooks = await BookModel.findById("6358d503e331dc18e375eb33");
    ...
    //b= b+ 10
    //console.log(b)
    //let savedData = await BookModel.get(data);
    res.send({msg: allBooks})
}
const year= async function (req, res) {

    let allBooks = await BookModel.find({year:"2022"})
res.send({ msg: allBooks});
}
const perticularebook = async function (req, res) {
...
module.exports.getBooksData = getBooksData
    ...
    module.exports.perticularebook=perticularebook
